import type { CampaignInfo } from '../types/frame';

export const CAMPAIGN_CONFIG: CampaignInfo = {
  candidateName: 'Pastor Ezequias',
  office: 'Deputado Estadual',
  slogan: 'Coragem, Fé e Trabalho por Você!',
  number: '15 333',
  hashtag: '#TôComPastorEzequias',
  shareMessage:
    '🇧🇷 Acabei de criar minha foto de perfil oficial apoiando o Pastor Ezequias para Deputado Estadual! Crie a sua também e fortaleça nossa campanha:',
  legalNotice:
    'ELEIÇÕES 2026 - EZEQUIAS ALBERTO SOUSA - DEPUTADO ESTADUAL - CNPJ CANDIDATO: 68.545.404/0001-02',
  instagramUrl: 'https://instagram.com',
  whatsappGroupUrl: 'https://chat.whatsapp.com',
};

export const DEFAULT_TRANSFORM = {
  scale: 1,
  offsetX: 0,
  offsetY: 0,
  rotation: 0,
  flipHorizontal: false,
};

export const EXPORT_SIZE = 1080; // Standard 1080x1080px HD square avatar
